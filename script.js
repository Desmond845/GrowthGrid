...
class StatsTicker {
  ...
  generateStatsList() {
    ...
    this.stats = [
      ...
      `📱 Using GrowthGrid: ${this.getUsageDays()} ${
        this.getUsageDays() > 1 ? "Days" : "Day"
      }`,
  ...
  getTodaysEntries() {
    ...
    const aspects = JSON.parse(
      localStorage.getItem("growthGrid-aspects") || "[]"
    );
    let count = 0;
...
class SearchEngine {
  ...
  displayResults(results, query) {
    ...
    let html = `
            ...
            <h4>Found ${results.length} aspect${
      results.length > 1 ? "s" : ""
    } containing "${query}"</h4>
            </div>
        `;
    ...
    results.forEach((result) => {
      ...
      console.log(aspect);
      html += `
                <div class="search-result-item" onclick="openAspectlog('${
                  aspect.id
                }', '${aspect.name}')">
                    <div class="result-aspect">
                        <h5>${aspect.name}</h5>
                        <span class="match-count">${totalMatches} match${
        totalMatches !== 1 ? "es" : ""
      }</span>
                    </div>
                    ...
                    ${entryMatches
                      ...
                      .map(
                        (entry) => `
                        <div class="match-item">
                            <span class="match-type">Entry • ${formatDateHeader(
                              entry.date
                            )} ${entry.entryTime}</span>
...
class SmartImporter {
  ...
  validateImportedData(data) {
    ...
    if (data.app && !data.app.includes("GrowthGrid")) {
      ...
      const proceed = confirm(
        "⚠️ This file doesn't seem to be from GrowthGrid. Import anyway?"
      );
      return proceed;
    }
  ...
  showImportPreview(importedData) {
    const currentAspects = JSON.parse(
      localStorage.getItem("growthGrid-aspects") || "[]"
    );
  ...
  countTotalEntries(aspects) {
    return aspects.reduce((total, aspect) => {
      return (
        total +
        (aspect.entries?.reduce(
          (sum, day) => sum + (day.entriesToday?.length || 0),
          0
        ) || 0)
      );
    }, 0);
  ...
  replaceData(importedData) {
    ...
    localStorage.setItem(
      "growthGrid-aspects",
      JSON.stringify(importedData.aspects)
    );
    ...
    if (importedData.userName) {
      document.getElementById(
        "welcomeName"
      ).textContent = `Welcome ${importedData.userName}`;
    }
  ...
  mergeDayEntries(existingDay, importedDay) {
    ...
    existingDay.entriesToday.forEach((entry) => {
      existingEntriesSet.add(
        entry.text.toLowerCase().trim() + "|" + entry.time
      );
    });
    ...
    importedDay.entriesToday.forEach((importedEntry) => {
      const key =
        importedEntry.text.toLowerCase().trim() + "|" + importedEntry.time;
  ...
  loggingPage.innerHTML = `
    <section class="logging-page">
        <header class="log-header">
            <button onclick="closeAspectLog('${aspectId}')"><svg class="back-icon" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
// Close aspect log
function closeAspectLog(id) {
  // Show dashboard, hide logging page
  history.pushState(null, `Log: nothing`);
    ...
    <div class="day-group" data-animate="up" data-stagger="${
      (dayIndex % 3) + 1
        ...
        ${dayEntry.entriesToday
          ...
          <button class="copy-entry-btn" onclick="copyEntryText('${
            entry.text
          }', event)">
  ...
  modal.innerHTML = `
    <div class="modal-content stats-content">
      <div class="stats-header">
        ...
        <button class="close-btn" onclick="this.closest('.modal-overlay').remove(); document.body.classList.remove('modal-active')
">×</button>
      </div>
...
<div class="stat-item">
          ...
          <span class="stat-value">${statsTicker.getUsageDays()} ${
    statsTicker.getUsageDays() > 1 ? "Days" : "Day"
  } 📅</span>
        </div>
        <div class="stat-item">
      ...
      <div class="stats-actions">
        <button class="close-stats-btn" onclick="this.closest('.modal-overlay').remove();     document.body.classList.remove('modal-active')
 ">
          Awesome!
        </button>
      </div>
  ...
  try {
    ...
    const totalEntries = aspects.reduce((total, aspect) => {
      return (
        total +
        (aspect.entries?.reduce(
          (sum, day) => sum + day.entriesToday.length,
          0
        ) || 0)
      );
    }, 0);
    ...
    const interval = setInterval(() => {
      ...
      progressBar.update(
        percent,
        `Processed ${processed} of ${totalToProcess} items`
      );
      ...
      if (processed >= totalToProcess) {
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = `growthgrid-${new Date().toLocaleDateString(
          "en-CA"
        )}.json`;
        document.body.appendChild(a);
        a.click();
...
function isDuplicateEntry(entry1, entry2) {
  ...
  const sameText =
    entry1.text.trim().toLowerCase() === entry2.text.trim().toLowerCase();
...
function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    ...
    navigator.serviceWorker
      .register("/GrowthGrid/sw.js", { scope: "/GrowthGrid/" })
      .then((registration) => {
...
function setupBackButtonNavigation() {
  ...
  window.addEventListener("popstate", function (event) {
    if (event.state === null) {
      // We're at the initial page (dashboard)
      showDashboard();
    } else if (event.state.page === "logging") {
      // We're going back to logging page
      const { aspectId, aspectName } = event.state;
      openAspectlog(aspectId, aspectName);
    } else {
  ...
  // Also handle Android back button (Cordova/Capacitor)
...
function handleMobileBackButton() {
  ...
  if (loggingPage && loggingPage.style.display === "block") {
    // If logging page is open, close it
    closeAspectLog();
